<?php
    ...
    function getPortada(){
        ...
        $res = $mysqli->query("SELECT nombre, path_foto1, id FROM cientificos WHERE publicado=1 ORDER BY id") ;
        $portada = array() ;
    ...
    function getCientifico($idCf){
        ...
        compruebaId($idCf);
        $res = $mysqli->query("SELECT nombre, lugar, fecha, path_foto1, pie_foto1, path_foto2,
            pie_foto2, descripcion1, descripcion2, wikipedia, publicado FROM cientificos WHERE id=" . $idCf);
        if($res->num_rows > 0){
            $row = $res->fetch_assoc();
            $cientifico = array(
                'id' => $idCf,
    ...
    function getListaCientificos(){
        ...
        $res = $mysqli->query("SELECT id, nombre, fecha, lugar, path_foto1, publicado FROM cientificos ORDER BY id") ;
        $lista = array() ;
    ...
    function modifyCientifico($idCientifico, $nombre, $lugar, $fecha,
        ...
        if(isset($publicado)) $mysqli->query("UPDATE cientificos SET publicado='$publicado' Where id='$idCientifico'");
    }

    // Buscamos los cientificos cuyo nombre coincide
    function searchCientificoCompleto($valor){
        global $mysqli;
        ...
        while($row = $resultado->fetch_assoc()){
            ...
            array_push($cientificos,$datos);
        }

        return json_encode($cientificos);
    }
    function searchCientifico($valor){
        global $mysqli;
        $mysqli->real_escape_string($valor); //Limpiamos los caracteres problematicos
        $cientificos = array();

        $resultado = $mysqli->query(" SELECT id, nombre FROM cientificos 
            WHERE nombre LIKE '%$valor%' AND publicado='1' ") or die("???");
